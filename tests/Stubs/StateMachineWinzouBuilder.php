    ...
    public function from()
    {
        ...
        foreach ($transitions as $event => $transition){
            ...
            foreach ($from as $state){
                ...
                $sm_array[$state][$event] = [ $transition['to'] ];

                if( in_array($transition['to'], $guard_to) )
                    $sm_array[$state][$event] = [ $transition['to'], StateMachine::EXEC_GUARD => $guard ];
                if( in_array($state, $before_from) )
                    $sm_array[$state][$event] = [ $transition['to'], StateMachine::EXEC_BEFORE => $before ];
                if( in_array($transition['to'], $after_on) )
                    $sm_array[$state][$event] = [ $transition['to'], Statemachine::EXEC_AFTER => $after ];
            }
        }
